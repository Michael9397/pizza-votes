...
<div class="min-h-screen bg-gradient-to-b from-orange-50 to-white">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="py-12 text-center">
            ...
            @else
                <div class="space-x-4">
                    ...
                    <a href="{{ route('register') }}" class="inline-block px-6 py-3 bg-white text-orange-600 border border-orange-600 rounded-lg hover:bg-orange-50 transition">
                        Sign Up
                    </a>
                </div>
            @endauth
        ...
        @if ($restaurants->count() > 0)
            <div class="mb-16">
                <h2 class="text-2xl font-bold text-gray-900 mb-8">Recent Ratings</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach ($restaurants as $restaurant)
                        <div class="bg-white rounded-lg shadow hover:shadow-lg transition p-6">
                                ...
                                <div class="space-y-2 mb-4">
                                    @foreach ($restaurant['dimension_scores'] as $dimension => $score)
                                        <div class="flex items-center justify-between text-sm">
                                            <span class="text-gray-600 capitalize">{{ ucfirst($dimension) }}</span>
                            ...
                            <!-- Vote Button -->
                            @auth
                                <a href="{{ route('restaurants.show', $restaurant['id']) }}" class="w-full block text-center px-4 py-2 bg-orange-100 text-orange-700 rounded hover:bg-orange-200 transition font-medium">
                                    View Details & Vote
                                </a>
                            @else
                                <a href="{{ route('restaurants.vote', $restaurant['id']) }}" class="w-full block text-center px-4 py-2 bg-orange-100 text-orange-700 rounded hover:bg-orange-200 transition font-medium">
                                    Vote
                                </a>
                            @endauth
                        </div>
